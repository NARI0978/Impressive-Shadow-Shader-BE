...
void main( in PS_Input PSInput, out PS_Output PSOutput )
{
	...
	float3 CC_NC = float3(0.2,0.21,0.25);
	float4 n_color = PSInput.color;
	float weather = smoothstep(0.8,1.0,FOG_CONTROL.y);//天候時
	n_color = lerp(lerp(n_color,FOG_COLOR,0.33)+float4(0.0,0.05,0.1,0.0),FOG_COLOR*1.1,smoothstep(0.1,0.4,PSInput.sky));

	float day = smoothstep(0.15,0.25,FOG_COLOR.g);//日中
	float3 cc = lerp(CC_NC,CC_DC,day);//雲の色
	float lb = lerp(0.0,0.55,weather);//雲の量
	float cm = fBM(10,lb,1.2,PSInput.pos.xz*3.5 -TIME*0.01);//雲の動き
